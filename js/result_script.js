...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!resultData.fromHistory) {
        ...
        // Trigger automatic Telegram backup right after submitting an exam (placed outside SQLite block)
        if (sessionStorage.getItem('triggerExamSubmitBackup') === 'true') {
            sessionStorage.removeItem('triggerExamSubmitBackup');
            setTimeout(() => {
                executeOnSubmitBackup();
            }, 1000); // Small delay
        }
    } else {
...
// ============================================
// Immediate Telegram Backup on Submit
// ============================================
async function executeOnSubmitBackup() {
    ...
    try {
        // Use exportAllData() from db.js
        const data = await window.exportAllData();
        const jsonStr = JSON.stringify(data);
        const file = new Blob([jsonStr], { type: 'application/json' });

        const timestamp = new Date().toISOString().replace(/:/g, '-').split('.')[0];
        const title = data.exams && data.exams.length > 0 ? data.exams[0].title.replace(/[^a-zA-Z0-9_-]/g, '_') : 'Exam';

        const formData = new FormData();
        formData.append('chat_id', chatId);
        formData.append('document', file, `Backup_${title}_${timestamp}.json`);
        formData.append('caption', `✅ Fresh Backup saved upon exam completion.`);
        ...
        if (response.ok) {
            console.log('✅ Automated post-exam Telegram backup successful.');
            // Also update the daily tracker so it doesn't double-fire if they go back to dashboard
            const today = new Date().toISOString().split('T')[0];
            localStorage.setItem('lastTelegramBackup', today);
        } else {
            console.error('Failed post-exam Telegram backup:', await response.text());
        }
    } catch (e) {
        console.error('Error during post-exam Telegram backup:', e);
    }
}
